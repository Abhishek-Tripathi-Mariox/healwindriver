...
const buildInfo = (p: any): [string, string][] => {
  const driver = p?.driver ?? p ?? {};
  const vehicle = driver.activeVehicle ?? driver.vehicle ?? (Array.isArray(driver.vehicles) ? driver.vehicles[0] : undefined) ?? {};
  const mobile = driver.mobileNumber || driver.phone || driver.mobile || '';
  const license = driver.drivingLicenseNumber || driver.licenseNumber || driver.dlNumber || '';
  const dob = fmtDate(driver.dob || driver.dateOfBirth);
  const reg = vehicle.registrationNumber || vehicle.regNumber || vehicle.number || '';
  return ([
...
export const ProfileScreen: React.FC = () => {
  ...
  const photoUrl = firstPhotoUrl(driver.profilePhoto ?? profile?.profilePhoto);
  ...
  const changePhoto = async () => {
    ...
    try {
      ...
      setPhotoBusy(true);
      await uploadProfilePhoto('driver', file);
    } catch (e: any) {
      AppAlert.alert('Photo not updated', e?.message || 'Could not update your photo. Please try again.');
    } finally {
      setPhotoBusy(false);
    }
  };
